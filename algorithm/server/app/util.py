...
def build_adj_list():
    ...
    # get qs_table ==> tensor(num_q, num_s)
    ...
    for q_id in range(num_question):
        s_ids = np.reshape(np.argwhere(
            qs_table[q_id] > 0), [-1]).tolist()  # 每个问题涉及的技能id
...
def gen_gikt_graph(q_neighbors_list, s_neighbors_list, q_neighbor_size=4, s_neighbor_size=10):
    ...
    q_neighbors = np.zeros(
        [num_question, q_neighbor_size], dtype=np.int32)  # 问题的邻居矩阵
    s_neighbors = np.zeros([num_skill, s_neighbor_size],
                           dtype=np.int32)  # 技能的邻居矩阵
    for i, neighbors in enumerate(q_neighbors_list):
        ...
        if len(neighbors) >= q_neighbor_size:  # 邻居数量超过或等于最大限制,随机选取一些, 但[无重复]
            q_neighbors[i] = np.random.choice(
    ...
    for i, neighbors in enumerate(s_neighbors_list):
        ...
        if len(neighbors) >= s_neighbor_size:
            s_neighbors[i] = np.random.choice(
                neighbors, s_neighbor_size, replace=False)
            continue
        if len(neighbors) > 0:
            s_neighbors[i] = np.random.choice(
                neighbors, s_neighbor_size, replace=True)
