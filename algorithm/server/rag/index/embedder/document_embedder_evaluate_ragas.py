    ...
    def __init__(self) -> None:
        ...
        if self.llm_name == 'OpenAI':
            ...
            embeddings = ZhipuAIEmbeddings(
                api_key="6e5294aa63fa49d186f994c849e1def8.detSDLFawmdzZKV2",
                model=ZHIPUAI_EMBEDDING_MODEL_NAME)
        elif self.llm_name == 'Ollama':
            base_url = os.getenv('OLLAMA_BASE_URL')
            embeddings = OllamaEmbeddings(
                base_url=base_url, model=OLLAMA_EMBEDDING_MODEL_NAME)
        ...
        for item in data:
            ...
            for part_index, part_content in enumerate(chunk_text_vec):
                metadata: Dict[str, str] = {
                    "source": url, "id": f"{doc_source}-{doc_id}-part{part_index}"}
            ...
            if documents_to_add:
                ...
                records_to_add.append((doc_id, doc_source, json.dumps(
                    embedding_id_vec), timestamp, timestamp))
